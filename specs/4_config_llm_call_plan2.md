...
5. **What to flag** — flag issues *only* when one of the universal rules is violated. If the configuration is reasonable, return zero findings. Do not invent issues to seem useful.
...
`utils/openai.ts` exposes a generic caller — roughly `callLLM<T>(systemPrompt: string, userPrompt: string, schema: ZodSchema<T>, opts: { model: string; temperature?: number }): Promise<T>` — so route and validator code never touch the OpenAI SDK directly. The validator code in `llm-config-validator/` uses this caller. Endpoint input validation lives in `routes/`.

If Zod ever proves limiting (e.g., the OpenAI helper hits an edge case we cannot work around), fall back to AJV with hand-written JSON Schema files in `schemas/`.

Manual testing happens in a notebook (`.ipynb`) the user maintains locally. The notebook **must** include the three I/O examples from `specs/assignment.md` as test cases against the running endpoint.
...
2. **`overall_verdict` was dropped (Option 2).** OpenAI's `strict: true` cannot enforce cross-field invariants like "findings empty iff verdict is fine." Keeping the field would create a new failure mode where the LLM disagrees with itself. Deriving the branch from `findings.length` removes the conflict by construction. Lost: an explicit "I think this is fine" self-declaration beyond the array length. Recovered from `findings.length === 0` and `verdict_confidence`.
